...
def mapping_tab_keywords(mapping_sheet: str) -> List[str]:
    ...
    for row in values[1:]:
        if idx < len(row):
            kws.extend(
                x.strip()
                for x in re.split(r"[|,，;；]", norm(row[idx]))
                if x.strip()
            )
...
def load_mapping_text(mapping_sheet: str) -> str:
    try:
        ...
        raise RuntimeError(
            f"规则库中不存在供应商Mapping：{mapping_sheet}"
        ) from e
...
def detect_supplier(file_name: str) -> Tuple[str, str]:
    ...
    if not hits:
        raise RuntimeError(
            ...
            f"当前Mapping Tab：{', '.join(cached_mapping_sheet_names())}"
        )
    ...
    if len(hits) > 1 and hits[0][0] == hits[1][0]:
        raise RuntimeError(
            f"供应商识别冲突：{hits[0][1]} / {hits[1][1]}；"
            f"请修改Mapping中的供应商识别关键词。"
        )
...
def gemini_upload_file(
    ...
    r = requests.post(
        f"{GEMINI_UPLOAD}?key={AI_API_KEY}",
        headers=headers,
        json=meta,
        timeout=60
    )
    ...
    r.raise_for_status()
    return r.json().get("file") or r.json()


def gemini_wait_file(
    file_name: str,
    timeout: int = 120
) -> Dict[str, Any]:
    ...
    while time.time() < end:
        r = requests.get(
            f"{GEMINI_BASE}/{file_name}?key={AI_API_KEY}",
            timeout=30
        )
...
def ai_extract_full_excel(
    ...
    if not file_name_api or not file_uri:
        raise RuntimeError(
            f"Gemini Files API上传成功但缺少文件引用：{file_info}"
        )
...
2. Mapping的每个字段为一项任务，请完成“是否AI读取”为“是”的字段内容，根据Mapping和{file_name}逐一输出每个字段内容。对某具体字段来说，在{file_name}中，从“Sheet名称”sheet的“Sheet列名”列、“Sheet行名”行中，在其“内容定位”中检索“提取内容”，将“提取内容”按照“映射规则”转换，最后“输出格式”是输出字段内容的格式要求，“示例”为该字段内容的格式示意，不是最终输出的字段内容。若某字段无可用信息，按“默认值”输出。
...
6. 对RMB /kg、RMB /parcel两个字段来说，根据具体的Weight (kg)值，按照Mapping在{file_name}中查找字段RMB /kg、字段RMB /parcel的对应值，若不存在对应值则返回null，不得猜测或杜撰。
7. Supplier固定为“{supplier}”；Destination Countryr固定为“{target_country}”。
...
"""
        ...
        r = requests.post(
            f"{GEMINI_BASE}/models/{AI_MODEL}:generateContent?key={AI_API_KEY}",
            json=body,
            timeout=300
        )
        ...
        if not r.ok:
            ...
            raise RuntimeError(
                f"Gemini API错误：{r.status_code} - {detail}"
            )
...
def ensure_header(ws):
    ...
    if not is_header:
        ...
        return (
            STANDARD_FIELDS,
            [(no + 1, row) for no, row in numbered]
        )
    ...
    if missing:
        ...
        ws.update(
            f"{col_letter(start)}1:"
            f"{col_letter(start + len(missing) - 1)}1",
            [missing],
            value_input_option="RAW"
        )
...
def build_record(
    ...
    return {
        ...
        "Time Min (day)": route.get("Time Min (day)"),
        "Time Max (day)": route.get("Time Max (day)"),
        "Time Type (workday/nature day)": norm(
            route.get("Time Type (workday/nature day)")
        ),
        "Volume Limit (cm)": norm(
            route.get("Volume Limit (cm)")
        ),
        "Volume to Weight Parameter": route.get(
            "Volume to Weight Parameter"
        ),
        "Weight (kg)": item.get("Weight (kg)"),
        "RMB /kg": item.get("RMB /kg"),
        "RMB /parcel": item.get("RMB /parcel"),
        "Pick&Packing/parcel": route.get(
            "Pick&Packing/parcel"
        ),
        "RMB in total": None,
        "USD in total": None,
        "DDP": norm(route.get("DDP")),
        "Extra Tax Required": norm(
            route.get("Extra Tax Required")
        ),
        "Tax Policy": norm(
            route.get("Tax Policy")
        ),
    }
...
def write_results(
    ...
    for record in records:
        ...
        if is_new:
            ...
            for a, b in runs:
                updates.append({
                    "range": (
                        f"{col_letter(a + 1)}{row_no}:"
                        f"{col_letter(b + 1)}{row_no}"
                    ),
                    "values": [
                        row_values[a:b + 1]
                    ]
    ...
    # 关键修复：
    # 这里在 for record in records 外面统一执行
    if updates:
        ws.batch_update(
            updates,
...
if uploaded is not None:
    ...
    try:
        ...
        supplier, evidence = detect_supplier(
            uploaded.name
        )
        ...
        st.write(
            f"Mapping：{supplier} ｜ "
            f"Excel Sheet：{len(all_sheets)}个"
        )
        ...
        if not target_country:
            ...
            with st.spinner(
                "Gemini正在读取完整Excel并按照Mapping提取……"
            ):

                result = ai_extract_full_excel(
                    file_hash,
                    file_bytes,
                    uploaded.name,
                    supplier,
            ...
            if failed:
                ...
                with st.expander(
                    ...
                    for route in failed:
                        ...
                        st.write(
                            f"{route.get('sheet', '')}: "
                            f"{'；'.join(route.get('errors', []))}"
                        )

            if (
                ok_routes
                and st.button(
                    "写入目标数据表（Google Sheets）"
                )
            ):

                updates, appends = write_results(
                    target_country,
                    ok_routes
                )
